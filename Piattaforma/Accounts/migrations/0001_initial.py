# Generated by Django 5.1 on 2024-09-24 07:09

import django.db.models.deletion
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('Users', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='Conto',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('nome', models.CharField(max_length=50)),
                ('saldo', models.DecimalField(decimal_places=2, default=0, max_digits=10)),
                ('tipo', models.CharField(choices=[('corrente', 'Conto Corrente'), ('investimento', 'Conto di Investimento'), ('risparmio', 'Conto di Risparmio')], default='corrente', max_length=20)),
            ],
        ),
        migrations.CreateModel(
            name='IntestazioniConto',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('data_intestazione', models.DateField()),
                ('conto', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='Accounts.conto')),
                ('utente', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='Users.utente')),
            ],
            options={
                'unique_together': {('utente', 'conto')},
            },
        ),
    ]
